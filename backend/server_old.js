...
// CORS Configuration
// Allow all origins for testing/Render deployment as requested
app.use(cors({
    origin: '*',
    credentials: true
}));
...
const connectDB = async () => {
    try {
        ...
        // Explicitly log failure for Render logs
        console.error('CRITICAL: Could not connect to MongoDB. Check MONGO_URI and IP Whitelist.');

        // Fallback logic (User requested to know if it fails, but we keep fallback for now to avoid crash loop)
